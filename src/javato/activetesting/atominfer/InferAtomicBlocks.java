...
public class InferAtomicBlocks {
    ...
    private Map<UniqueEvent, Map<String, List<UniqueEvent>>> sortedRaceRelation;
	...
	final Map<String, ArrayList<UniqueEvent>> sortedRacingEventsByThread = initDataStructures();
    ...
    private Map<String, ArrayList<UniqueEvent>> initDataStructures() {
    ...
    private Map<String, ArrayList<UniqueEvent>> getSortedRacingEventsByThread(Map<UniqueEvent, Set<UniqueEvent>> raceRelation) {
	Map<String, ArrayList<UniqueEvent>> racingEventsByThread = new HashMap<String, ArrayList<UniqueEvent>>();
    ...
    private Map<UniqueEvent, Map<String, List<UniqueEvent>>> getSortedRaceRelation(Map<UniqueEvent, Set<UniqueEvent>> raceRelation) {
	Map<UniqueEvent, Map<String, List<UniqueEvent>>> sortedRaceRelation = new HashMap<UniqueEvent, Map<String, List<UniqueEvent>>>();
	for (UniqueEvent from: raceRelation.keySet()) {
	    Map<String, List<UniqueEvent>> curMap = new HashMap<String, List<UniqueEvent>>();
	    sortedRaceRelation.put(from, curMap);
	    for (UniqueEvent to: raceRelation.get(from)) {
	    	String toThread = to.getThread();
    ...
    private void findPaths(final Map<String, ArrayList<UniqueEvent>> sortedRacingEventsByThread, int numThreads) {
	...
	for (String thread: sortedRacingEventsByThread.keySet()) {
	...
	HashMap<String, Integer> threadEventIndexMap = new HashMap<String, Integer>();
    ...
    private Result findPath(UniqueEvent cur, UniqueEvent start, UniqueEvent dest, int numThreads, Stack<UniqueEvent> curPath, HashMap<String, Integer> threadEventIndexMap) {
	//System.out.println("Exploring event index " + cur.getEventIndex() + " with cur path " + curPath);
    	String curThread = cur.getThread();
	...
	if (immediateSequentialRelation.containsKey(cur))
		...
		return Result.PATH_FOUND;
	// Check race relation
	Map<String, List<UniqueEvent>> racingEvents = sortedRaceRelation.get(cur);
	for (String nextThread: racingEvents.keySet()) {
    ...
    private Result backtrack(Stack<UniqueEvent> curPath, HashMap<String, Integer> threadEventIndexMap, String curThread, Integer initialEventIndex) {
